...
class TaskController extends GetxController {
  ...
  List<double> progressData = List<double>.filled(7, 0.0); // Initialize empty data
  ...
  void loadTasks() async {
    try {
      ...
      for (var task in tasks) {
        final taskDate = task[DatabaseHelper.columnUpdatedDate] as String;
        final isChecked = task[DatabaseHelper.columnIsChecked] as int;
        final dateKey = DateFormat('yyyy-MM-dd').format(DateTime.parse(taskDate));
  ...
  // Getter method to check if tasks are completed on a specific date
  bool isTaskCompletedOnDate(DateTime date) {
    final dateKey = DateFormat('yyyy-MM-dd').format(date);
    return completionDates[dateKey] ?? false; // Returns false if the date is not found
  }
