...
class TaskController extends GetxController {
 ...
 // List<double> progressData = List<double>.filled(7, 0.0);
    ...
    try {
      ...
      int updateId = await db.insert(
        DatabaseHelper.tableTaskTrack,
        {
          ...
          DatabaseHelper.columnTrackedDate: DateFormat('dd-MM-yyyy').format(DateTime.now()),
        },
      );
  ...
  Future<List<double>> fetchProgressDataForTrackedDate(String trackedDate) async {
    List<double> progressData = List.filled(7, 0.0);

    for (int i = 0; i < 7; i++) {
      DateTime date = DateTime.parse(trackedDate).subtract(Duration(days: i));
  ...
  Future<void> updateTaskStatus(int index, bool isTracked) async {
    try {
      ...
      String trackedDate = DateFormat('yyyy-MM-dd').format(DateTime.now());
  ...
  Future<List<double>> fetchProgressDataForLastDays(int days) async {
    try {
      ...
      final tasks =
      await _databaseHelper.getTasks(); // Fetch tasks from the database
      List<double> progressData =
      List<double>.filled(days, 0.0); // Initialize progress data with zeros

      await Future.forEach(tasks, (task) async {
        final taskCompletionDate =
        task['updated_date']; // Fetch the updated date (completion date)
